...
export const ProfileSettings: FC<ProfileSettingsProps> = ({}) => {
  ...
  useEffect(() => {
    const fetchDefaultApiKey = async () => {
      try {
        ...
        const defaultKey = data.defaultOpenaiApiKey || ""
        setDefaultOpenaiAPIKey(defaultKey)

        // 如果用户没有设置API key，使用默认值
        if (!profile?.openai_api_key && defaultKey) {
  ...
  const handleSave = async () => {
    ...
    const updatedProfile = await updateProfile(profile.id, {
      ...
      openai_api_key: openaiAPIKey || defaultOpenaiAPIKey,
        ...
        <div className="mt-6 flex items-center">
          <div className="flex items-center space-x-1">
            ...
            <WithTooltip
              display={
                <div>
                  Download MentalShield Project data as JSON. Import coming
                  soon!
                </div>
              }
              trigger={
